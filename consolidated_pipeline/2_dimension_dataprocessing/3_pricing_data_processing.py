...
# coalesce function returns the first non Null value, so if our record is 2025-01-12, then the first try_to_date will return a NULL, same with second try_to_date, but the third one returns the valid date format
...
# If the gross_price value is less than 0, then multiply it by -1 to make it positive, otherwise, let it be the same but cast it to double
...
# get the year from month column, and name it year, and another column named is_zero which is 0 if gross_price is not zero and 1 if gross_price is zero

# Use window function to group by product_code and year, and order by is_zero and month desc, and get the latest month's price and save it as df_gold_latest_price
